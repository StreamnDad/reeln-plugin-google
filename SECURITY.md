...
reeln-plugin-google is pre-1.0 software. Security fixes are published
against the latest release only. We recommend always running the most
recent version from
[PyPI](https://pypi.org/project/reeln-plugin-google/) or the
[Releases page](https://github.com/StreamnDad/reeln-plugin-google/releases).
...
reeln-plugin-google is a reeln-cli plugin that integrates Google
services for YouTube livestreaming, video uploads, playlist management,
and comments. It runs inside `reeln-cli` on a livestreamer's local
machine and makes outbound HTTPS requests to the Google APIs using
OAuth 2.0 credentials stored on disk.

In-scope concerns include, but are not limited to:
- Leakage of OAuth client secrets, access tokens, or refresh tokens via
...
- Scope abuse — requesting broader YouTube API scopes than necessary,
  or failing to honor revoked tokens
- Unsafe deserialization of Google API responses or cached metadata
- Command injection or path traversal in generated upload artifacts or
  metadata written to disk
- Dependency confusion or typosquatting on the PyPI package name

Out of scope:
- Vulnerabilities in the YouTube / Google APIs themselves or in the
  upstream `google-api-python-client` / `google-auth` libraries —
...
1. Go to the [Security tab](https://github.com/StreamnDad/reeln-plugin-google/security)
   of this repository
...
- The version of reeln-plugin-google, reeln-cli, and Python you tested
  against
